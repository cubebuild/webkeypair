...
export async function publicKeyToAddress(key: CryptoKey) {
	...
	assert(address.length === 20, `expected address to be 20 bytes, got ${address.length}`);
...
export async function signObject(key: CryptoKey, object: Record<string, unknown>) {
